...
class ConversationSession(models.Model):
    ...
    title = models.CharField(
        max_length=255,
        help_text="AI-generated title summary of the conversation"
    )
...
class Conversation(models.Model):
    ...
    def __str__(self):
        session_title = self.session.title if self.session else 'N/A'
        timestamp_str = self.timestamp.strftime('%Y-%m-%d %H:%M')
        return f"Conversation from {timestamp_str} (Session: {session_title})"
...
class Video(models.Model):
    ...
    video_url = models.URLField(
        help_text="URL to the video (e.g., a YouTube link)"
    )
