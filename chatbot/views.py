...
# from django.utils.encoding import smart_bytes, smart_str  # for some cases
    ...
    logic_adapters=[
        {
            ...
            'default_response': (
                (
                    (
                        "I'm here to listen. Could you tell me more about what"
                        " you're experiencing?"
                    )
                )
            ),
...
def chatbot_view(request):
    ...
    conversations_yesterday = Conversation.objects.filter(
        user=request.user,
...
# Therapist Recommendation
    if "therapist" in user_input_lower or "recommend" in user_input_lower:
        ...
        if therapists:
            ...
            response_text = (
                "I'm sorry, I don't have any therapist recommendations at the "
                "moment."
            )
...
def get_cached_therapists():
    ...
    if not therapists:
        ...
        cache.set(
            'therapist_recommendations', therapists, timeout=3600
        )  # Cache for 1 hour
    return therapists
...
def get_cached_videos():
    ...
    if not videos:
        videos = list(
            Video.objects.only('title', 'description', 'video_url')[:1]
        )
        cache.set(
            'video_recommendations', videos, timeout=3600
        )  # Cache for 1 hour
    return videos
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if user_input.lower() == 'exit':
            print(
                "Bot: Take care of yourself. I'm always here when you need to "
                "talk."
            )
            break
...
def load_session_view(request):
    ...
    try:
        ...
        messages_data = []
        for conv in convs:
            # You can decide what to include here – for now,
            # include type and text
